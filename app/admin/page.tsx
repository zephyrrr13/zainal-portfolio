    ...
    <div className="space-y-8">
      ...
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between border-b border-white/10 pb-6">
        ...
        <div className="flex items-center gap-3">
          <button
            ...
            disabled={loading}
            className="flex items-center gap-2 rounded-xl border border-white/20 bg-zinc-900 px-4 py-2.5 font-mono text-xs text-white hover:bg-white/10 transition-colors disabled:opacity-50"
          >
          ...
          <Link
            href="/admin/articles"
            className="flex items-center gap-2 rounded-xl border border-white bg-white px-4 py-2.5 font-mono text-xs font-bold uppercase text-black hover:bg-zinc-200 transition-colors"
          >
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <div className="rounded-2xl border border-white/10 bg-zinc-950/80 p-5 backdrop-blur-xl">
          <div className="flex items-center justify-between">
            <span className="font-mono text-xs uppercase tracking-wider text-zinc-400">Total Pageviews</span>
          ...
          <div className="mt-4 flex items-baseline gap-2">
            ...
            <span className="flex items-center font-mono text-xs text-emerald-400">
              <TrendingUp className="h-3 w-3 mr-0.5" /> +14.2%
            </span>
          </div>
          <p className="mt-1 font-mono text-[10px] text-zinc-500">Kunjungan akumulatif 30 hari terakhir</p>
        </div>

        {/* Metric 2 */}
        <div className="rounded-2xl border border-white/10 bg-zinc-950/80 p-5 backdrop-blur-xl">
          <div className="flex items-center justify-between">
            <span className="font-mono text-xs uppercase tracking-wider text-zinc-400">Pengunjung Unik</span>
          ...
          <p className="mt-1 font-mono text-[10px] text-zinc-500">Berdasarkan unique IP address</p>
        </div>

        {/* Metric 3 */}
        <div className="rounded-2xl border border-white/10 bg-zinc-950/80 p-5 backdrop-blur-xl">
          <div className="flex items-center justify-between">
            <span className="font-mono text-xs uppercase tracking-wider text-zinc-400">Rata-Rata Durasi</span>
          ...
          <div className="mt-4 flex items-baseline gap-2">
            ...
            <span className="font-mono text-xs text-zinc-400">Target: 3m+</span>
          </div>
          <p className="mt-1 font-mono text-[10px] text-zinc-500">Waktu eksplorasi portofolio & gallery</p>
        </div>

        {/* Metric 4 */}
        <div className="rounded-2xl border border-white/10 bg-zinc-950/80 p-5 backdrop-blur-xl">
          <div className="flex items-center justify-between">
            <span className="font-mono text-xs uppercase tracking-wider text-zinc-400">Status Keamanan</span>
            <div className="rounded-lg bg-emerald-500/10 p-2 text-emerald-400">
              <ShieldCheck className="h-4 w-4" />
            </div>
          </div>
          <div className="mt-4 flex items-baseline gap-2">
            <span className="text-3xl font-black tracking-tight text-emerald-400">100% SECURE</span>
          </div>
          <p className="mt-1 font-mono text-[10px] text-zinc-500">Vibe Check & Google SMTP Active</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
        ...
        <div className="rounded-3xl border border-white/10 bg-zinc-950/80 p-6 lg:col-span-2">
          <div className="flex items-center justify-between mb-6">
            <div>
              ...
              <p className="font-mono text-xs text-zinc-400">Aktivitas pageviews harian real-time</p>
            </div>
            <div className="font-mono text-xs text-zinc-400">Minggu Ini</div>
          </div>
            ...
            {data?.dailyChart.map((d, i) => {
              ...
              return (
                <div key={i} className="group relative flex flex-1 flex-col items-center gap-2 h-full justify-end">
                  <div className="opacity-0 group-hover:opacity-100 transition-opacity absolute -top-8 rounded bg-white px-2 py-0.5 font-mono text-[10px] font-bold text-black pointer-events-none shadow">
                    {d.views} views
                  </div>
                  <div
                  ...
                  />
                  <span className="font-mono text-[10px] uppercase text-zinc-400">{d.day}</span>
                </div>
              );
            })}
          </div>
        ...
        <div className="rounded-3xl border border-white/10 bg-zinc-950/80 p-6 flex flex-col justify-between">
          <div>
            ...
            <p className="font-mono text-xs text-zinc-400">Persentase pengunjung berdasarkan device</p>
          </div>
          ...
          <div className="rounded-xl border border-white/5 bg-zinc-900/60 p-3 font-mono text-[11px] text-zinc-400">
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
        ...
        <div className="rounded-3xl border border-white/10 bg-zinc-950/80 p-6">
          ...
          <p className="font-mono text-xs text-zinc-400 mb-4">Halaman yang paling sering dikunjungi</p>

          <div className="space-y-3">
            {data?.topPages.map((page, i) => (
              <div
                key={i}
                className="flex items-center justify-between rounded-xl border border-white/5 bg-zinc-900/50 px-4 py-2.5 font-mono text-xs"
              >
                <span className="text-zinc-200 font-bold">{page.path}</span>
                <span className="text-zinc-400">{page.count} views</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="rounded-3xl border border-white/10 bg-zinc-950/80 p-6">
          ...
          <p className="font-mono text-xs text-zinc-400 mb-4">Asal platform pengunjung website</p>
          ...
          <div className="space-y-3">
            {data?.topReferrers.map((ref, i) => (
              <div
                ...
                className="flex items-center justify-between rounded-xl border border-white/5 bg-zinc-900/50 px-4 py-2.5 font-mono text-xs"
              >
                <span className="text-zinc-200">{ref.source}</span>
